  ...
  public static void stateDillo() {
    if (canDillo() && ArmadilloStates.isOnline() && !isSmartTP) {
      new Thread(() -> {
        ...
        // ThreadUtils.threadSleepRandom will choose a number between -20, 20 and add it to the time of sleep (yes i do have (-) checks)
    ...
    if (ids.mc.thePlayer.isRiding()) {
      new Thread(() -> {
        ...
        if (ArmadilloStates.isOnline()) {
          ...
          // Armadillo basically does not let u jump untill it fals to the ground.
          ThreadUtils.threadSleepRandom(ping * 2L);

          // Yes. Ik terrible function call bla bla bla i suck at codong :<
          newSpinDrive();
        } else {
